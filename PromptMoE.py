...
class PairRouter(nn.Module):
    ...
    @torch.no_grad()
    def gating_weights(self, g, method="pair_marginal", T=1.0, T_pair=1.0, topk=2, light_mask_indices=None):
        ...
        if light_mask_indices is not None:
            indices_dev = light_mask_indices.to(logits.device)
            mask = indices_dev.unsqueeze(0).expand(B, -1)
            logits.scatter_(1, mask, float("-inf"))
...
@torch.no_grad()
def get_weights_from_pair_router(router: PairRouter, context_g: torch.Tensor, topk: int, light_mode: bool = False) -> Dict[str, float]:
    method = "utility" if topk == 1 else "pair_marginal"

    light_mask_indices = None
    if light_mode:
        # Indices corresponding to: Gd (1), frac (7), Sm (8), prompt (9)
        light_mask_indices = torch.tensor([1, 7, 8, 9], device=context_g.device, dtype=torch.long)

    w = router.gating_weights(context_g, method=method, topk=topk, 
                             light_mask_indices=light_mask_indices)
...
def process_model(
    ...
    for sub in tqdm(subdirs, desc=f"Processing {model_name}"):
        ...
        if manual_weights_str:
            ...
            selected_weights = get_weights_from_pair_router(router, g, topk=router_topk, light_mode=light_mode)
